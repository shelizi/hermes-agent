...
def _profile_live_catalog(normalized: str) -> Optional[list[str]]:
    ...
    if profile.auth_type == "external_process":
        try:
            live = profile.fetch_models(timeout=12.0)
            if live:
                return live
        except Exception:
            pass
...
def _credential_fingerprint(provider: str) -> str:
    ...
    if provider in {"devin-acp", "devin", "devin-cli"}:
        ...
        devin_paths = [
            os.path.join(appdata, "devin", "credentials.toml") if appdata else "",
            os.path.join(xdg, "devin", "credentials.toml") if xdg else "",
            os.path.join(home, ".config", "devin", "credentials.toml"),
            os.path.join(home, "Library", "Application Support", "devin", "credentials.toml"),
            os.path.join(localappdata, "devin", "cli", "model_configs_v4.bin") if localappdata else "",
            os.path.join(localappdata, "devin", "cli", "model_configs_v2.bin") if localappdata else "",
            os.path.join(localappdata, "devin", "cli", "model_configs.bin") if localappdata else "",
        ]
        for p in devin_paths:
            if p:
                _mtime_part(p, p)
    ...
    if provider in {"grok-acp", "grok-cli", "grok-build", "xai-grok-cli"}:
        home = os.path.expanduser("~")
        for p in (os.path.join(home, ".grok", "auth.json"), os.path.join(home, ".grok", "config.toml")):
            _mtime_part(p, p)
