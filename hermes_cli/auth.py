...
class ProviderConfig:
    ...
    id: str
    name: str
    auth_type: str  # "oauth_device_code", "oauth_external", "oauth_minimax", "external_process", "api_key", ...
    portal_base_url: str = ""
...
_REGISTRY_ROWS: Tuple[Any, ...] = (
    ...
    ProviderConfig(
        "grok-acp", "Grok CLI ACP", "external_process",
        inference_base_url="acp://grok", base_url_env_var="GROK_ACP_BASE_URL"),
    ProviderConfig(
        "antigravity-acp", "Google Antigravity ACP", "external_process",
        inference_base_url="acp://antigravity", base_url_env_var="ANTIGRAVITY_ACP_BASE_URL"),
    ProviderConfig(
        "codex-acp", "OpenAI Codex CLI ACP", "external_process",
        inference_base_url="acp://codex", base_url_env_var="CODEX_ACP_BASE_URL"),
...
def _register_plugin_provider(pp: Any) -> None:
    ...
    if pp.auth_type == "external_process":
        base_url_var = next((v for v in pp.env_vars if is_url(v)), None) or "" if pp.env_vars else ""
...
def _resolve_external_process_command_path(
    ...
    if command is None:
        pconfig = PROVIDER_REGISTRY.get(provider_id)
        if pconfig:
            command, _, _, _, _ = _external_process_spec(pconfig)
    ...
    try:
        ...
        if profile and hasattr(profile, "search_command_path"):
...
def _external_process_spec_dict(provider_id: str) -> Dict[str, Any]:
...
def _external_process_auth_present(provider_id: str) -> Optional[bool]:
    ...
    try:
        ...
        profile = get_provider_profile(provider_id)
        if profile and hasattr(profile, "auth_present"):
...
def _external_process_spec(
    ...
    base_url = (_provider_env_base_url(cfg) or cfg.inference_base_url) if cfg else ""
    try:
    ...
    spec = _external_process_spec_dict(provider_id)
    command_env_vars = tuple(
        getattr(profile, "process_command_env_vars", ())
        or spec.get("command_env")
        or ()
    )
    args_env_var = str(
        getattr(profile, "process_args_env_var", "")
        or spec.get("args_env")
        or ""
    )
    default_cmd = str(
        getattr(profile, "process_command", "")
        or spec.get("default_command")
        or ""
    )
    command = (
        next((v for v in (os.getenv(var, "").strip() for var in command_env_vars) if v), "")
        or default_cmd
    )
    raw_args = os.getenv(args_env_var, "").strip() if args_env_var else ""
    default_args = list(
        getattr(profile, "process_args", ())
        or spec.get("default_args")
        or []
    )
    args = shlex.split(raw_args) if raw_args else default_args
    if profile and hasattr(profile, "resolve_command_args"):
        try:
    ...
    resolved_command = (
        _resolve_external_process_command_path(provider_id, command) if command else None
    )
...
def _resolve_external_process_command_args(provider_id: str) -> tuple[str, list[str]]:
    ...
    command, args, _, _, _ = _external_process_spec(provider_id)
    return command, args
