...
_PROVIDER_ERROR_MARKERS = (
    r"api\s+(?:call\s+)?failed", r"billing\s+or\s+credits\s+exhausted", r"provider\s+authentication\s+failed",
    r"non-retryable\s+error", r"rate\s+limited\s+after\s+\d+\s+retries", r"error\s+code\s*:", r"http\s*\d{3}\b",
