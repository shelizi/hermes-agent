...
def _provider_special_cases(c: _Ctx) -> Optional[Verdict]:
    ...
    if (
        ("could not start" in msg and "acp" in msg and ("command" in msg or "cli" in msg))
        or ("acp" in msg and "did not expose stdin/stdout" in msg)
        or ("unexpected argument" in msg and ("--acp" in msg or "acp" in msg))
    ):
        return _V_FORMAT_ERROR
    if "devin auth login" in msg or ("acp" in msg and "not logged in" in msg):
        return _v(_R.auth_permanent, retryable=False, should_fallback=False)
    return None
