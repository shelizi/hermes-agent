...
def _should_stream(agent: Any) -> bool:
    ...
    checks); disabled on provider signal, MoA/ACP without a display consumer, or
    Mock clients in tests (SimpleNamespace, not stream iterators)."""
    if getattr(agent, "_disable_streaming", False):
        return False
    client = getattr(agent, "client", None)
    if getattr(client, "prefers_sync_without_consumers", False) and not agent._has_stream_consumers():
        return False
